...
TEST(StartupRuntimeTest, CopiesDataZerosBssAndCallsMain) {
    ...
    EXPECT_TRUE(board.hasValue()) << "loads synthetic C-runtime startup fixture";
    if (!board) return;
    ...
    EXPECT_TRUE(data && bss && sentinel) << "startup fixture exposes verification symbols";
    if (!data || !bss || !sentinel) return;

    const auto initial_data = board.value()->memory().read32(*data);
    EXPECT_TRUE(initial_data && initial_data.value() == 0U)
        << "runtime .data begins at reset value before startup copy";
    for (std::uint32_t offset = 0; offset < 16U; offset += 4U) {
        EXPECT_TRUE(board.value()->memory().write32(*bss + offset, 0xa5a5a5a5U).hasValue())
            << "pre-dirties startup .bss";
    }
    EXPECT_TRUE(board.value()->memory().write32(*sentinel, 0x11111111U).hasValue())
    ...
    EXPECT_TRUE(result.instructions == 61U)
    ...
    for (std::uint32_t index = 0; index < 3U; ++index) {
        ...
        EXPECT_TRUE(value && value.value() == expected_data[index])
            << "startup copies one .data initializer from its flash LMA";
    }
    for (std::uint32_t offset = 0; offset < 16U; offset += 4U) {
        const auto value = board.value()->memory().read32(*bss + offset);
        EXPECT_TRUE(value && value.value() == 0U) << "startup zeros one pre-dirtied .bss word";
    }
    const auto final_sentinel = board.value()->memory().read32(*sentinel);
    EXPECT_TRUE(final_sentinel && final_sentinel.value() == 0xfeedbeefU)
