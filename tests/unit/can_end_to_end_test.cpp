...
public:
    TempCanWorldConfig() {
        const auto fixture = fil::test::fixtureBoardConfig("receiver");
        std::string board = "{\n"
            "  \"schema_version\": 1,\n"
            "  \"name\": \"receiver\",\n"
            ...
            "}\n";
        board_path_ = directory_.write("receiver.json", board);
    }
...
private:
    fil::test::TemporaryDirectory directory_{"fil-can-end-to-end-tests"};
...
std::uint32_t readRegister(fil::stm32g4::FdcanPeripheral& controller,
    ...
    EXPECT_TRUE(value.hasValue()) << "reads an FDCAN register";
...
void writeRegister(fil::stm32g4::FdcanPeripheral& controller, const std::uint32_t offset,
    ...
    EXPECT_TRUE(controller.write(offset, fil::mem::AccessSize::word, value, {}).hasValue())
...
TEST(CanEndToEndTest, ScheduledExternalFrameReachesMessageRamAndNvic) {
    ...
    EXPECT_TRUE(receiver->operational()) << "starts the receiving FDCAN core";
    ...
    EXPECT_TRUE((readRegister(*receiver, fil::stm32g4::FdcanPeripheral::rxf0sOffset) & 0x0fU) == 0U)
    ...
    EXPECT_TRUE((status & 0x0fU) == 1U && ((status >> 16U) & 0x3U) == 1U)
        << "advances the operational receiver's RX FIFO 0 state";
    EXPECT_TRUE((readRegister(*receiver, fil::stm32g4::FdcanPeripheral::irOffset) &
    ...
    EXPECT_TRUE(nvic_pending && (nvic_pending.value() & (1U << 21U)) != 0U)
    ...
    EXPECT_TRUE(traced_external_transmit && traced_receiver_delivery && traced_controller_receive)
        << "CAN-FD bus and controller traces use encoded DLC, decoded length, and qualified "
           "sources";
}
