...
public:
    TempWorldTimeConfigs()
        : root_(std::filesystem::temp_directory_path() / "fil-world-time-tests") {
...
bool sameLoopState(const fil::cpu::CpuState& left, const fil::cpu::CpuState& right) {
    return left.r == right.r && left.xpsr == right.xpsr
        && left.msp == right.msp && left.psp == right.psp
        && left.primask == right.primask && left.basepri == right.basepri
        && left.faultmask == right.faultmask && left.control == right.control
        && left.thumb == right.thumb && left.halted == right.halted
        && left.pending_exception == right.pending_exception
        && left.pending_exc_return == right.pending_exc_return
        && left.s == right.s && left.fpscr == right.fpscr
        && left.it_state == right.it_state
        && left.instruction_address == right.instruction_address;
}

void boardCountDoesNotScaleGlobalTime() {
    ...
    fil::test::check(one.hasValue() && three.hasValue(),
    ...
    fil::test::check(one_result.hasValue() && three_result.hasValue(),
                     "runs one- and three-board virtual-time worlds");
    if (!one_result || !three_result) return;

    fil::test::check(
        one_result.value().end_time_ns == three_result.value().end_time_ns,
    ...
    fil::test::check(
        one_result.value().boards[0].result.instructions == 3U
    ...
    fil::test::check(every_board_matches,
                     "each concurrent board receives the same CPU progress and completion time");
    fil::test::check(
        three_result.value().cycles == 3U * one_result.value().cycles,
        "world cycle totals remain the sum of independent board cycles"
    );
}

void timeBudgetProgressIsIndependentOfBoardCount() {
    TempWorldTimeConfigs files;
    const auto alpha = files.writeBoard("alpha.json", "alpha");
    const auto beta = files.writeBoard("beta.json", "beta");
    const auto gamma = files.writeBoard("gamma.json", "gamma");
    ...
    auto three = fil::sim::World::load(files.network({alpha, beta, gamma}));
    if (!one || !three) {
        fil::test::check(false, "loads worlds for concurrent time-budget test");
        return;
    }
    ...
    fil::test::check(
        one_result.value().reason == fil::sim::WorldStopReason::time_budget
    ...
    fil::test::check(equal_progress,
                     "time limiting does not divide per-board instructions by board count");
}

void concurrentScheduleIsByteDeterministic() {
    TempWorldTimeConfigs files;
    const auto alpha = files.writeBoard("alpha.json", "alpha");
    const auto beta = files.writeBoard("beta.json", "beta");
    ...
    if (!first || !second) {
        fil::test::check(false, "loads repeated worlds for virtual-time determinism test");
        return;
    }
    ...
    fil::test::check(first_result.hasValue() && second_result.hasValue(),
    ...
    fil::test::check(
        starts == std::vector<std::string>{
            "0:alpha", "0:beta", "62:alpha", "62:beta", "125:alpha", "125:beta",
        },
        "same-time CPU starts use stable configuration order at each virtual frontier"
    );
}

void sharedEventsKeepTheirExactDeadlines() {
    TempWorldTimeConfigs files;
    const auto alpha = files.writeBoard("alpha.json", "alpha");
    const auto beta = files.writeBoard("beta.json", "beta");
    auto world = fil::sim::World::load(files.network({alpha, beta}));
    if (!world) {
        fil::test::check(false, "loads world for shared event-deadline test");
        return;
    }
    ...
    fil::test::check(result.hasValue() && callback_time == 100U,
    ...
    fil::test::check(deadline_index < next_instruction_index,
                     "due events settle before either CPU starts at the next completion frontier");
}

void acceleratedPhaseMismatchedLoopsMatchExactExecution() {
    TempWorldTimeConfigs files;
    const auto alpha = files.writeBoard("alpha.json", "alpha");
    const auto beta = files.writeBoard("beta.json", "beta");
    const auto config = files.network({alpha, beta});
    ...
    if (!exact || !accelerated) {
        fil::test::check(false, "loads worlds for accelerated-loop equivalence");
        return;
    }
    fil::test::check(
        installIdleLoop(*exact.value(), "alpha", 0U)
    ...
    fil::test::check(exact_result.hasValue() && accelerated_result.hasValue(),
                     "runs exact and accelerated loop worlds");
    if (!exact_result || !accelerated_result) return;

    fil::test::check(
        exact_result.value().reason == accelerated_result.value().reason
    ...
    fil::test::check(
        sameLoopState(exact.value()->board("alpha")->cpu().state(),
                      accelerated.value()->board("alpha")->cpu().state())
            && sameLoopState(exact.value()->board("beta")->cpu().state(),
                             accelerated.value()->board("beta")->cpu().state()),
        "loop batching preserves full CPU state across a scheduled wakeup"
    );
    fil::test::check(
        exact.value()->trace().jsonLines() == accelerated.value()->trace().jsonLines(),
...
void acceleratedLoopsPreserveSysTickAndExceptionEntry() {
    ...
    auto accelerated = fil::sim::World::load(config);
    if (!exact || !accelerated) {
        fil::test::check(false, "loads worlds for SysTick batching equivalence");
        return;
    }
    fil::test::check(
        configureSysTickIdleLoop(*exact.value(), "tick-alpha", 1U)
    ...
    fil::test::check(
        accelerated_result.value().rounds < exact_result.value().rounds,
        "SysTick equivalence exercises a real accelerated path"
    );
    fil::test::check(
        sameLoopState(exact.value()->board("tick-alpha")->cpu().state(),
                      accelerated.value()->board("tick-alpha")->cpu().state())
            && sameLoopState(exact.value()->board("tick-beta")->cpu().state(),
...
void runWorldTimeTests() {
    boardCountDoesNotScaleGlobalTime();
    timeBudgetProgressIsIndependentOfBoardCount();
