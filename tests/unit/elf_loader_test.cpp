...
TEST(ElfLoaderTest, LoadsSplitAddressImage) {
    const auto image = fil::elf::load(fixture_path);
    EXPECT_TRUE(image.hasValue()) << "loads synthetic ELF fixture";
    if (!image) {
        return;
    }

    EXPECT_TRUE(image.value().entryPoint() == 0x08000009U) << "reads ELF entry point";
    EXPECT_TRUE(image.value().initialMsp() == 0x20020000U) << "reads initial MSP";
    EXPECT_TRUE(image.value().resetHandler() == 0x08000009U) << "reads reset vector";
    EXPECT_TRUE(image.value().hardFloatAbi()) << "reads hard-float EABI flag";
    EXPECT_TRUE(image.value().segments().size() == 3) << "reads all PT_LOAD segments";

    const auto& data_segment = image.value().segments()[1];
    EXPECT_TRUE(data_segment.physical_address == 0x08000018U) << "preserves .data load address";
    EXPECT_TRUE(data_segment.virtual_address == 0x20000000U) << "preserves .data runtime address";
    ...
    EXPECT_TRUE(!image.value().readLoadImage(0x20000000U, 4))
        << "does not confuse .data runtime address with load address";
}

/// @brief Verifies symbol and ARM build-attribute parsing.
TEST(ElfLoaderTest, LoadsSymbolsAndAttributes) {
    const auto image = fil::elf::load(fixture_path);
    if (!image) {
        EXPECT_TRUE(false) << "fixture is available for metadata test";
        return;
    }

    const fil::elf::ElfSymbol* reset = image.value().symbolAtOrBefore(0x0800000aU);
    EXPECT_TRUE(reset != nullptr && reset->name == "Reset_Handler")
        << "finds nearest function symbol";
    EXPECT_TRUE(reset != nullptr && reset->function) << "preserves function symbol type";
    EXPECT_TRUE(reset != nullptr && reset->address == 0x08000008U)
        << "normalizes the Thumb bit in function symbol addresses";
    EXPECT_TRUE(image.value().armAttributes().cpu_name == std::optional<std::string>("Cortex-M4"))
        << "reads ARM CPU name attribute";
    EXPECT_TRUE(image.value().armAttributes().thumb_isa_use == 2U) << "reads Thumb-2 attribute";
    EXPECT_TRUE(image.value().armAttributes().fp_arch == 6U) << "reads FPv4-D16 attribute";
    EXPECT_TRUE(image.value().armAttributes().vfp_args == 1U) << "reads VFP argument attribute";
}
...
TEST(ElfLoaderTest, RejectsMalformedImages) {
    ...
    EXPECT_TRUE(!bad_magic && bad_magic.error().message.find("magic") != std::string::npos)
        << "rejects bad ELF magic";
    EXPECT_TRUE(!bad_sizes && bad_sizes.error().message.find("p_filesz") != std::string::npos)
        << "rejects p_filesz greater than p_memsz";
    EXPECT_TRUE(!overlap && overlap.error().message.find("overlap") != std::string::npos)
        << "rejects conflicting load-address overlap";
    EXPECT_TRUE(!wrap && wrap.error().message.find("wraps") != std::string::npos)
        << "rejects target address wraparound";
    EXPECT_TRUE(!short_file && short_file.error().message.find("smaller") != std::string::npos)
